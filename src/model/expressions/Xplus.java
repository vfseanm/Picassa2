...
public class Xplus implements Expression {
    private Expression myOperand1;
    private Expression myOperand2;

    public Xplus(String cmd, ArrayList<Expression> ops) {
        if  (ops.size()!=2)
    ...
    public static ExpressionFactory getFactory() {
        return new ExpressionFactory(new Xplus());
    }

    public boolean isThisKindofThing(String cmd) {
        if (cmd.contentEquals("+") || cmd.contentEquals("plus") )
            return true;
        return false;
