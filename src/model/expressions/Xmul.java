...
public class Xmul implements Expression {
    private Expression myOperand1;
    private Expression myOperand2;

    public Xmul(String cmd, ArrayList<Expression> ops) {
        if  (ops.size()!=2)
    ...
    public static ExpressionFactory getFactory() {
        return new ExpressionFactory(new Xmul());
    }

    public boolean isThisKindofThing(String cmd) {
        if (cmd.contentEquals("*") || cmd.contentEquals("mul") )
            return true;
        return false;
